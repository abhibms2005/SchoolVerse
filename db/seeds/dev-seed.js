...
function seedDemo(db) {
  ...
  /* ---------- fee ledger (Day 4) ----------
     Balance-driven mode for two students: Rohan owes (expected 45k, paid 15k
     → 30k overdue — matches his seeded fee_status and drives the fee_overdue
     notification), Diya is settled (45k paid in full). Others keep manual
     fee_status mode (expected_fee = 0), so their seeded flags stay put. */
  const { reconcileStudentFees } = require('../../src/fees');
  db.prepare(`UPDATE students SET expected_fee = 45000 WHERE id = ?`).run(students[2]); // Rohan
  db.prepare(`UPDATE students SET expected_fee = 45000 WHERE id = ?`).run(students[1]); // Diya
  const pay = db.prepare(`INSERT INTO payments (student_id, amount, date, method, note) VALUES (?, ?, date('now', '-2 months'), 'bank_transfer', ?)`);
  pay.run(students[2], 15000, 'partial payment');
  pay.run(students[1], 45000, 'annual fee');
  reconcileStudentFees(db, students[2]); // → overdue (30k balance)
  reconcileStudentFees(db, students[1]); // → paid (0 balance)
  ...
  console.log(`│  fees: ${db.prepare('SELECT COUNT(*) c FROM payments').get().c} payments · 2 students balance-driven`);
  console.log(`│  absences: 1 student at the 3-day alert threshold`);
